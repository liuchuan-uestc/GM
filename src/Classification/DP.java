...
public class DP{
	...
	public static double DPMain(String strDir, Map<String, Map<Integer, Double>> TrainFileMapVsm, 
								           Map<String, Map<Integer, Double>> TestFileMapVsm,
								           Map<String,String> actual, Map<String,String> pred) throws IOException {
		double accuracy = CBCDragPushing(strDir, TrainFileMapVsm, TestFileMapVsm, actual, pred);
		return accuracy;
	}

	public static double CBCDragPushing(String strDir, Map<String, Map<Integer, Double>> TrainFileMapVsm, 
								           Map<String, Map<Integer, Double>> TestFileMapVsm,
								           Map<String,String> actual, Map<String,String> pred) throws IOException {
		...
		for (Iterator<Map.Entry<String, Map<Integer, Double>>> it = CateCenterMapSet1.iterator(); it.hasNext();) {
			Map.Entry<String, Map<Integer, Double>> me = it.next();
			CateCenterFactorMap.put(me.getKey(),1.0);
		}

		CateCenterMap = DragPushingForCentriod(TrainFileMapVsm, TestFileMapVsm, CateCenterMap);
		TestResult(CateCenterFactorMap, TestFileMapVsm, CateCenterMap,actual, pred);
		return 0;
	}
	
   //DragPushing策略
	public static Map<String, Map<Integer, Double>> DragPushingForCentriod(Map<String, Map<Integer, Double>> TrainFileMapVsm, 
								           	     Map<String, Map<Integer, Double>> TestFileMapVsm,
								          		 Map<String, Map<Integer, Double>> CateCenterMap) throws IOException {
		Map<String, Map<Integer, Double>> CateCenterSaveMap = new TreeMap<String, Map<Integer, Double>>();
		Map<String, Double> CateCenterFactorMap = new TreeMap<String, Double>();
		Set<Map.Entry<String, Map<Integer, Double>>> CateCenterMapSet = CateCenterMap.entrySet();
		for (Iterator<Map.Entry<String, Map<Integer, Double>>> it = CateCenterMapSet.iterator(); it.hasNext();) {
			Map.Entry<String, Map<Integer, Double>> me = it.next();
		...
		CateCenterSaveMap = DataPreProcess.copyMaptoMap(CateCenterMap);
		for(int n=0; ((n<6)||(lift>0.001)&&(n<50)); n++)
		{
			Oldaccray = Newaccray;
			System.out.println("----------------First step "+n+", lift: "+lift+", learnrate"+learnrate+"----------------");
			...
			for (Iterator<Map.Entry<String, Map<Integer, Double>>> it = TrainFileMapVsmSet.iterator(); it.hasNext();) {
				...
				for (Iterator<Map.Entry<String, Map<Integer, Double>>> it2 = CateCenterMapSet1.iterator(); it2.hasNext();) {
					...
					TrainDistance[i][j] = getDistance(me.getValue(),me2.getValue());
				}
			    i++;
			}
			...
			for (Iterator<Map.Entry<String, Map<Integer, Double>>> it = TrainFileMapVsmSet2.iterator(); it.hasNext();) {
				...
				if(Integer.parseInt(Cate) == nearestMeans[i]){
					...
					for (Iterator<Map.Entry<Integer, Double>> it2 = allWordSet.iterator(); it2.hasNext();) {
						...
						if(CateCenterSaveMap.get(Cate).containsKey(me2.getKey()))
						{
							factor = CateCenterSaveMap.get(Cate).get(me2.getKey())+learnrate*me2.getValue();
							if(factor<0){
								factor = 0;
							}
							CateCenterSaveMap.get(Cate).put(me2.getKey(),factor);
						}
						
						if(CateCenterSaveMap.get(""+nearestMeans[i]).containsKey(me2.getKey()))
						{
							factor = CateCenterSaveMap.get(""+nearestMeans[i]).get(me2.getKey())-learnrate*me2.getValue();
							if(factor<0){
								factor = 0;
							}
							CateCenterSaveMap.get(""+nearestMeans[i]).put(me2.getKey(),factor);
						}
					}
				}
				i++;
			}
			CateCenterMap = DataPreProcess.copyMaptoMap(CateCenterSaveMap);
			if(true)
			{
				System.out.println("Train Accuracy = "+(double)correct/total*100+"% ("+correct+"/"+total+") (DragPushingForCentriod Classifier)\n");
	...
	public static double getDistance(Map<Integer, Double> map1,
		...
		//return EuclideanDistance(map1, map2);
